...
if __name__ == '__main__':
    ...
    dict1 = ReplDict()
    o= SyncObj('localhost:%d' % port, partners, consumers=[dict1])
    n = 0
    old_value = -1
    while True:
        # time.sleep(0.005)
        ...
        if n < 20000:
            dict1.
            if(dict1.get('testKey1') is None):
                dict1.set('testKey1', n, sync=True)
            else:
                i=dict1.get('testKey1')
                print(i)
                dict1.set('testKey1', dict1.get('testKey1')+n, sync=True)
                print(str(i) +"+"+str(n)+"="+str(dict1.get('testKey1')))
